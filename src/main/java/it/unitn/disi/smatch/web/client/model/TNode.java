...
public class TNode {
    ...
    public TNode(String nodeLabel, int level, TNode parent) {
        ...
        this._id = Document.get().createUniqueId();
    }
    ...
    public TNode getChildByLabel(String childLabel) {
    ...
    public TNode getChildById(String nodeId) {
        System.out.println("Node has children:" + hasChildren());
        if (!hasChildren()) return null;
        for (int i = 0; i < getChildrenSize(); i++) {
            System.out.println("this child:" + this.getChildAt(i).getid());
            if (nodeId.equalsIgnoreCase(this.getChildAt(i).getid())) {
                return getChildAt(i);
            } else {
                TNode deepChildNode = this.getChildAt(i).getChildById(nodeId);
                if (deepChildNode != null) {
                    return deepChildNode;
                }
            }
        }
        return null;
